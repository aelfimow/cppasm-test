...
TEST(Instruction5, Type6)
{
    m64 addr { RDX };

    KMOVQ(k1, k2);
    EXPECT_EQ(asmstr(), "kmovq %k2, %k1");
    KMOVQ(k1, addr);
    EXPECT_EQ(asmstr(), "kmovq (%rdx), %k1");
    KMOVQ(addr, k1);
    EXPECT_EQ(asmstr(), "kmovq %k1, (%rdx)");
    KMOVQ(k1, RAX);
    EXPECT_EQ(asmstr(), "kmovq %rax, %k1");
    KMOVQ(RAX, k2);
    EXPECT_EQ(asmstr(), "kmovq %k2, %rax");
}
